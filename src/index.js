...
const getTwitterProfiles = async (req, res, next) => {
  ...
  Promise.all(accounts.map(twitter.fetchTwitterProfile))
...
const updateTwitterProfiles = async (req, res, next) => {
  ...
  const accounts = res.body;
  Promise.all(accounts.map(mongo.updateTwitterProfile))
    .then(() => next());
};
...
const getTweetStats = async (req, res, next) => {
  ...
  const accounts = res.body;
  Promise.all(accounts.map(twitter.fetchTwitterStats))
    .then((updatedAccounts) => {
      res.body = updatedAccounts;
      next();
    });
...
const updateTwitterStats = async (req, res, next) => {
  ...
  const accounts = res.body;
  Promise.all(accounts.map(mongo.updateTwitterStats))
    .then(() => next());
};
